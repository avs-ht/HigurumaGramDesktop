...
#ifdef OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"HigurumaGram.HigurumaGramDesktop.Store";
#else // OS_WIN_STORE
const WCHAR AppUserModelIdBase[] = L"HigurumaGram.HigurumaGramDesktop";
#endif // OS_WIN_STORE
...
void CleanupShortcut() {
	...
	QString path = systemShortcutPath() + u"HigurumaGram.lnk"_q;
...
bool checkInstalled(QString path = {}) {
	...
	const auto installed = u"HigurumaGram Desktop/HigurumaGram.lnk"_q;
	const auto old = u"HigurumaGram for Windows/HigurumaGram.lnk"_q;
...
bool ValidateShortcut() {
	...
	if (cAlphaVersion()) {
		path += u"HigurumaGramAlpha.lnk"_q;
