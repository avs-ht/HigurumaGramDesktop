...
void processLegacy(const QString &iconPath) {
	...
	auto shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/HigurumaGram Desktop.lnk";
	if (!QFile::exists(shortcut)) {
		shortcut = appdata + "/Microsoft/Internet Explorer/Quick Launch/User Pinned/TaskBar/HigurumaGram.lnk";
	}
	if (!QFile::exists(shortcut)) {
		return;
	}
...
void processNewShortcuts(const QString &iconPath) {
	...
	const auto shortcuts = {
		path + u"HigurumaGram Desktop/HigurumaGram.lnk"_q,
		path + u"HigurumaGram/HigurumaGram.lnk"_q,
